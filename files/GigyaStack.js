...
if (!_.isUndefined(appName) && !_.isUndefined(environment)) {
    var creds = new AWS.SharedIniFileCredentials({profile: 'manoj'});
    ...
    opsworksHelper.createStack(defaultParams.defaultStackParams, appName, 't2.small', function(stackData) {
        if (stackData) {
            var isOnline = false;
